...
# if any lidar reading less than this, move away from that side
hallway_steering_thresh = 950.0
...
# use the middle section +/- this many sections for checking front collision
num_front_sect = 2
array_start = -90.0
array_stop = 90.0
...
# correcting error in magnetometer
...
while(run_flag):
    ...
    while(button_state):  # run until reset button pushed
        ...
        distances = cf.compute_lidar_array(
            lidar_scan, lidar_ang_arr, distances)
        if distances[mid_ind] > 0:  # check if lidar readings initialized
            # CHECK STATE MACHINE
            playing_audio, motor_command = cf.obstacle_state_machine(
                playing_audio, distances, mid_ind, m_offset, motor_gain, motor_max, deadband, target_heading, current_heading, motor_min, lidar_arr_mean, forward_thresh)
            # CHECK WHEN TO ALLOW AUDIO AGAIN
            playing_audio, prev_pa_flag, audio_start_time = cf.wait_audio(
                playing_audio, prev_pa_flag, audio_start_time, audio_delay)
        ...
        if loop_cnt % loop_num == 0:  # print how fast it is running
            ...
            print("state:", state, ' ', "goal yaw:", goal_yaw, ' ', "cur yaw:", cur_yaw, ' ', "fwd:", round(distances[mid_ind]), ' ', "Min:", round(
                np.amin(distances)), ' ', "Min ind:", np.argmin(distances)-mid_ind, ' ', 'Mot:', motor_command)
